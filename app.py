...
# Dicionário mestre com os itens organizados por Setor e Subcategoria
DADOS_SISTEMA = {
    "RESTAURANTE / COZINHA": {
...
# Usamos um DataFrame direto no Session State para permitir edição perfeita
...
# ➔ MÁGICA: Junta todos os itens de todas as categorias do setor em uma lista única e plana!
lista_itens_plana = []
...
if item_nao_cadastrado:
    ...
    item_bruto = st.selectbox("Busque e selecione o Item:", sorted(lista_itens_plana), key=f"item_select_{st.session_state.reset_counter}")
    item_nome_limpo, unidade_medida = separar_unidade(item_bruto)
    
    # ➔ DESCOBERTA AUTOMÁTICA: O sistema localiza a categoria correta sem o usuário ver
...
if st.button("➕ Adicionar Item ao Pedido", use_container_width=True):
    if nome_solicitante.strip() == "":
        ...
        # Junta o novo item na nossa tabela
...
if not st.session_state.carrinho_df.empty:
    ...
    st.caption("💡 Dica: Dê dois cliques na **Quantidade** ou **Observação** para alterar. Selecione a linha e aperte **Delete** no teclado para remover o item.")
    
    # ➔ O EDITOR INTERATIVO DE TABELAS: Bloqueia colunas fixas e libera Quantidade/Observação para edição
    st.session_state.carrinho_df = st.data_editor(
        st.session_state.carrinho_df,
        column_config={
            "Data_Hora": None, # Oculta data/hora para poupar espaço
            "Solicitante": None, # Oculta o solicitante na tabela para ficar limpo
        ...
        num_rows="dynamic" # Ativa a lixeira nativa do Streamlit para deletar linhas!
    )
    ...
    if st.button("🚀 ENVIAR REQUISIÇÃO PARA A CENTRAL", type="primary", use_container_width=True):
        if "COLE_AQUI" in URL_WEB_APP:
            st.error("Erro: Falta colocar a sua URL do Google Script na linha 12!")
        else:
            with st.spinner("Enviando dados diretamente para o Google Sheets..."):
                try:
                    # Converte o DataFrame editado de volta para JSON antes de despachar via internet
